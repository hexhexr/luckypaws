...
export default async function handler(req, res) {
  ...
  try {
    ...
    // This log confirmed: 'expires_at' is already in MILLISECONDS.
    ...
    // --- Invoice Expiry Time Handling (CORRECTED) ---
    let expiresAt = null;
    // We confirmed from the API response that 'expires_at' is ALREADY in MILLISECONDS.
    // So, NO MULTIPLICATION by 1000 is needed.
    if (typeof payment.expires_at === 'number' && payment.expires_at > 0) {
      expiresAt = payment.expires_at; // Use directly (already in milliseconds)
      ...
      if (!isNaN(parsedExpiresAt) && parsedExpiresAt > 0) {
        expiresAt = parsedExpiresAt; // Use directly (already in milliseconds)
      }
    }
    ...
    // Return expiresAt to the frontend
